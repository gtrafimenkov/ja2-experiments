...
typedef struct {
  ...
  RECT Region;
  LPDIRECTDRAWSURFACE pSurface1;
  LPDIRECTDRAWSURFACE2 pSurface2;
...
static LPDIRECTDRAWSURFACE gpFrameBuffer1 = NULL;
static LPDIRECTDRAWSURFACE2 gpFrameBuffer2 = NULL;
...
BOOLEAN InitializeVideoManager(struct PlatformInitParams *params) {
  ...
  if (!DDCreateSurface(gpDirectDrawObject, &SurfaceDescription, &gpFrameBuffer1, &gpFrameBuffer2)) {
    return FALSE;
  }
  ...
  SurfaceDescription.dwWidth = MAX_CURSOR_WIDTH;
  SurfaceDescription.dwHeight = MAX_CURSOR_HEIGHT;
  if (!DDCreateSurface(gpDirectDrawObject, &SurfaceDescription, &gpMouseCursorOriginal1,
                       &gpMouseCursorOriginal2)) {
    return FALSE;
  }
  ...
  for (uiIndex = 0; uiIndex < 1; uiIndex++) {
    ...
    if (!DDCreateSurface(gpDirectDrawObject, &SurfaceDescription,
                         &gMouseCursorBackground[uiIndex].pSurface1,
                         &gMouseCursorBackground[uiIndex].pSurface2)) {
      return FALSE;
    }
  }
    ...
    for (cnt = 0; cnt < usNumStrips; cnt++) {
      ...
      do {
        ReturnCode = IDirectDrawSurface2_BltFast(
            pDest, StripRegions[cnt].left, StripRegions[cnt].top, gpFrameBuffer2,
...
void RefreshScreen(void *DummyVariable) {
    ...
    do {
      ReturnCode =
          IDirectDrawSurface2_BltFast((LPDIRECTDRAWSURFACE2)vsBackBuffer->_platformData2,
                                      ...
                                      gMouseCursorBackground[CURRENT_MOUSE_DATA].pSurface2,
                                      (LPRECT)&Region, DDBLTFAST_NOCOLORKEY);
      if ((ReturnCode != DD_OK) && (ReturnCode != DDERR_WASSTILLDRAWING)) {
        DirectXAttempt(ReturnCode, __LINE__, __FILE__);
  ...
  if (guiFrameBufferState == BUFFER_DIRTY) {
    ...
    if (gfFadeInitialized && gfFadeInVideo) {
      ...
      if (gfForceFullScreenRefresh == TRUE) {
        ...
        do {
          ReturnCode =
              IDirectDrawSurface2_BltFast((LPDIRECTDRAWSURFACE2)vsBackBuffer->_platformData2, 0, 0,
                                          gpFrameBuffer2, (LPRECT)&Region, DDBLTFAST_NOCOLORKEY);
          if ((ReturnCode != DD_OK) && (ReturnCode != DDERR_WASSTILLDRAWING)) {
            DirectXAttempt(ReturnCode, __LINE__, __FILE__);
        ...
        for (uiIndex = 0; uiIndex < guiDirtyRegionCount; uiIndex++) {
          ...
          do {
            ReturnCode = IDirectDrawSurface2_BltFast(
                (LPDIRECTDRAWSURFACE2)vsBackBuffer->_platformData2, Region.left, Region.top,
                gpFrameBuffer2, (LPRECT)&Region, DDBLTFAST_NOCOLORKEY);
            if ((ReturnCode != DD_OK) && (ReturnCode != DDERR_WASSTILLDRAWING)) {
              DirectXAttempt(ReturnCode, __LINE__, __FILE__);
            }
      ...
      if ((Region.right > Region.left) && (Region.bottom > Region.top)) {
        ...
        do {
          ReturnCode = IDirectDrawSurface2_BltFast(
              ...
              (LPDIRECTDRAWSURFACE2)vsBackBuffer->_platformData2, &Region, DDBLTFAST_NOCOLORKEY);
          if ((ReturnCode != DD_OK) && (ReturnCode != DDERR_WASSTILLDRAWING)) {
            DirectXAttempt(ReturnCode, __LINE__, __FILE__);
          }
  ...
  if (gfRenderScroll) {
    ...
    do {
      ReturnCode = IDirectDrawSurface2_BltFast(
          (LPDIRECTDRAWSURFACE2)vsBackBuffer->_platformData2, 0, 0,
          (LPDIRECTDRAWSURFACE2)vsPrimary->_platformData2, &Region, DDBLTFAST_NOCOLORKEY);
      if ((ReturnCode != DD_OK) && (ReturnCode != DDERR_WASSTILLDRAWING)) {
        DirectXAttempt(ReturnCode, __LINE__, __FILE__);
  ...
  if (gMouseCursorBackground[PREVIOUS_MOUSE_DATA].fRestore == TRUE) {
    ...
    do {
      ReturnCode = IDirectDrawSurface2_BltFast(
          (LPDIRECTDRAWSURFACE2)vsBackBuffer->_platformData2,
          gMouseCursorBackground[PREVIOUS_MOUSE_DATA].usMouseXPos,
...
BOOLEAN Set8BPPPalette(struct SGPPaletteEntry *pPalette) {
  ...
  ReturnCode = IDirectDrawSurface_SetPalette(gpFrameBuffer2, gpDirectDrawPalette);
  if (ReturnCode != DD_OK) {
