...
BOOLEAN EnterSaveLoadScreen() {
  ...
  CHECKF(AddStandardVideoObject(&VObjectDesc, &guiSlgBackGroundImage));

  // load Load Screen Add ons graphic and add it
  VObjectDesc.fCreateFlags = VOBJECT_CREATE_FROMFILE;
  GetMLGFilename(VObjectDesc.ImageFile, MLG_LOADSAVEHEADER);
  CHECKF(AddStandardVideoObject(&VObjectDesc, &guiBackGroundAddOns));
...
void RenderSaveLoadScreen() {
  ...
  if (gfSaveGame) {
    ...
    BltVideoObjectOld(vsIndexFB, hPixHandle, 1, SLG_TITLE_POS_X, SLG_TITLE_POS_Y);
  } else {
    // If we are Loading a game

    // Display the Title
    GetVideoObject(&hPixHandle, guiBackGroundAddOns);
    BltVideoObjectOld(vsIndexFB, hPixHandle, 0, SLG_TITLE_POS_X, SLG_TITLE_POS_Y);
  }
...
BOOLEAN DisplaySaveGameEntry(int8_t bEntryID)  //, uint16_t usPosY )
{
  ...
  // background
  GetVideoObject(&hPixHandle, guiBackGroundAddOns);
  BltVideoObjectOld(vsIndexFB, hPixHandle, gbSaveGameSelectedLocation[bEntryID], usPosX, usPosY);
  ...
  if (bEntryID == 0 && gfSaveGame) {
    ...
    ShadowVideoSurfaceRect(vsIndexFB, usPosX, usPosY, usPosX + SLG_SAVELOCATION_WIDTH,
