...
BOOLEAN EnterSaveLoadScreen() {
  ...
  CHECKF(AddVObjectAndSetTransparency(CreateVideoObject(&VObjectDesc), &guiSlgBackGroundImage));

  // load Load Screen Add ons graphic and add it
  VObjectDesc.fCreateFlags = VOBJECT_CREATE_FROMFILE;
  GetMLGFilename(VObjectDesc.ImageFile, MLG_LOADSAVEHEADER);
  CHECKF(AddVObjectAndSetTransparency(CreateVideoObject(&VObjectDesc), &guiBackGroundAddOns));
