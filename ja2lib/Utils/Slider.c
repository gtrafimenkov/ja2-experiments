...
BOOLEAN InitSlider() {
  ...
  CHECKF(AddVObject(CreateVObjectFromFile("INTERFACE\\SliderBox.sti"), &guiSliderBoxImage));
...
int32_t AddSlider(uint8_t ubStyle, uint16_t usCursor, uint16_t usPosX, uint16_t usPosY,
  ...
  pNewSlider->usBackGroundColor = rgb32_to_rgb16(FROMRGB(255, 255, 255));
...
void RenderSliderBox(SLIDER *pSlider) {
  ...
  if (pSlider->uiFlags & SLIDER_VERTICAL) {
    ...
    if (!(pSlider->LastRect.iLeft == 0 && pSlider->LastRect.iRight == 0)) {
      ...
      BlitBufferToBuffer(vsSaveBuffer, vsFB, (uint16_t)pSlider->LastRect.iLeft,
    ...
    // save the new rect
    BlitBufferToBuffer(vsFB, vsSaveBuffer, (uint16_t)DestRect.iLeft, (uint16_t)DestRect.iTop, 8,
                       15);
  }
