...
BOOLEAN DrawTextToScreen(wchar_t* pStr, uint16_t LocX, uint16_t LocY, uint16_t usWidth, uint32_t ulFont,
                         uint8_t ubColor, uint8_t ubBackGroundColor, BOOLEAN fDirty, uint32_t FLAGS);
uint16_t IanWrappedStringHeight(uint16_t usPosX, uint16_t usPosY, uint16_t usWidth, uint8_t ubGap,
                              uint32_t uiFont, uint8_t ubColor, wchar_t* pString, uint8_t ubBackGroundColor,
                              BOOLEAN fDirty, uint32_t uiFlags);

BOOLEAN WillThisStringGetCutOff(int32_t iCurrentYPosition, int32_t iBottomOfPage, int32_t iWrapWidth,
