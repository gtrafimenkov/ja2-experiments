...
uint8_t gpSamSectorX[] = {SAM_1_X, SAM_2_X, SAM_3_X, SAM_4_X};
uint8_t gpSamSectorY[] = {SAM_1_Y, SAM_2_Y, SAM_3_Y, SAM_4_Y};
...
void DrawTownLabels(wchar_t *pString, wchar_t *pStringA, uint16_t usFirstX, uint16_t usFirstY) {
  ...
  SetFontDestBuffer(vsSB, MapScreenRect.iLeft + 2, MapScreenRect.iTop, MapScreenRect.iRight,
  ...
  while (pNode) {
    ...
    if ((iDirection != -1)) {
      if ((!fZoomFlag) ||
        ...
        BltVideoObject(vsFB, hMapHandle, (uint16_t)iDirection, iX, iY, VO_BLT_SRCTRANSPARENCY,
                       NULL);
    ...
    if (sDest != sSource) {
      if (PlayersBetweenTheseSectors(
        ...
        if (!fZoomFlag) {
          ...
          BltVideoObject(vsSB, hIconHandle, (uint16_t)iCounter, (int16_t)iX, (int16_t)iY,
                         VO_BLT_SRCTRANSPARENCY, NULL);
        } else {
          ...
          BltVideoObject(vsSB, hIconHandle, (uint16_t)iCounter, iX, iY, VO_BLT_SRCTRANSPARENCY,
                         NULL);
  ...
  if (iHelicopterVehicleId != -1) {
    ...
    if (pVehicleList[iHelicopterVehicleId].ubMovementGroup != 0) {
      ...
      */
      ...
      /*
         ...
         sY=sY-MAP_GRID_Y; sX=sX-MAP_GRID_X;
                                      ...
                                      GetScreenXYFromMapXYStationary(
         ((uint16_t)(pGroup->ubNextX)),((uint16_t)(pGroup->ubNextY)) , &sX, &sY ); sY=sY-MAP_GRID_Y;
                                      sX=sX-MAP_GRID_X;
      ...
      */
      ...
      BltVideoObject(vsFB, hHandle, HELI_ICON, x, y, VO_BLT_SRCTRANSPARENCY, NULL);
...
void DisplayDestinationOfHelicopter(void) {
  ...
  if (GetLengthOfPath(pVehicleList[iHelicopterVehicleId].pMercPath) > 1) {
    ...
    GetVideoObject(&hHandle, guiHelicopterIcon);
    BltVideoObject(vsFB, hHandle, HELI_SHADOW_ICON, x, y, VO_BLT_SRCTRANSPARENCY, NULL);
...
void BlitMineIcon(uint8_t sMapX, uint8_t sMapY) {
  ...
  LockVideoSurface(vsSB, &uiDestPitchBYTES);
...
void BlitMineText(uint8_t sMapX, uint8_t sMapY) {
  ...
  SetFontDestBuffer(vsSB, MAP_VIEW_START_X, MAP_VIEW_START_Y,
                    MAP_VIEW_START_X + MAP_VIEW_WIDTH + MAP_GRID_X,
...
void RenderIconsPerSectorForSelectedTown(void) {
  ...
  for (iCounter = 0; iCounter < 9; iCounter++) {
    ...
    for (iCurrentTroopIcon = 0; iCurrentTroopIcon < iTotalNumberOfTroops; iCurrentTroopIcon++) {
      ...
      BltVideoObject(vsFB, hVObject, (uint16_t)(iCurrentIcon), sX, sY, VO_BLT_SRCTRANSPARENCY,
                     NULL);
    }
  }
...
void ShowHighLightedSectorOnMilitiaMap(void) {
  ...
  if (sSectorMilitiaMapSector != -1) {
    ...
    // blt the object
    BltVideoObject(vsFB, hVObject, 0, sX, sY, VO_BLT_SRCTRANSPARENCY, NULL);
  }
...
void DisplayUnallocatedMilitia(void) {
  ...
  for (iCurrentTroopIcon = 0; iCurrentTroopIcon < iTotalNumberOfTroops; iCurrentTroopIcon++) {
    ...
    BltVideoObject(vsFB, hVObject, (uint16_t)(iCurrentIcon), sX, sY, VO_BLT_SRCTRANSPARENCY, NULL);
  }
}
...
void ShowSAMSitesOnStrategicMap(void) {
  ...
  for (iCounter = 0; iCounter < NUMBER_OF_SAM_SITES; iCounter++) {
    ...
    BltVideoObject(vsSB, hHandle, ubVidObjIndex, sX, sY, VO_BLT_SRCTRANSPARENCY, NULL);
...
void DrawMapBoxIcon(struct VObject *hIconHandle, uint16_t usVOIndex, uint8_t sMapX, uint8_t sMapY,
  ...
  if (!fZoomFlag) {
    ...
    BltVideoObject(vsSB, hIconHandle, usVOIndex, iX, iY, VO_BLT_SRCTRANSPARENCY, NULL);
...
void DrawBullseye() {
  ...
  BltVideoObject(vsSB, hHandle, 0, sX, sY, VO_BLT_SRCTRANSPARENCY, NULL);
}
...
void HideExistenceOfUndergroundMapSector(uint8_t ubSectorX, uint8_t ubSectorY) {
  ...
  ColorFillVideoSurfaceArea(vsSB, sScreenX + 1, sScreenY, sScreenX + MAP_GRID_X,
