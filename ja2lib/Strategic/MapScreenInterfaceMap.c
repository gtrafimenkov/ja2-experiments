...
uint32_t DrawMap(void) {
  ...
  if (!iCurrentMapSectorZ) {
    pDestBuf = (uint16_t *)VSurfaceLockOld(vsSaveBuffer, &uiDestPitchBYTES);
  ...
  while (pNode) {
    ...
    if ((iDirection != -1)) {
      if ((!fZoomFlag) ||
        ...
        BltVideoObject2(vsFB, hMapHandle, (uint16_t)iDirection, iX, iY, VO_BLT_SRCTRANSPARENCY,
                        NULL);

        if (!fUTurnFlag) {
          BltVideoObject2(vsFB, hMapHandle, (uint16_t)iArrow, iArrowX, iArrowY,
                          VO_BLT_SRCTRANSPARENCY, NULL);
    ...
    if (sDest != sSource) {
      if (PlayersBetweenTheseSectors(
        ...
        if (!fZoomFlag) {
          ...
          BltVideoObject2(vsSaveBuffer, hIconHandle, (uint16_t)iCounter, (int16_t)iX, (int16_t)iY,
                          VO_BLT_SRCTRANSPARENCY, NULL);
        } else {
          ...
          BltVideoObject2(vsSaveBuffer, hIconHandle, (uint16_t)iCounter, iX, iY,
                          VO_BLT_SRCTRANSPARENCY, NULL);
  ...
  if (iHelicopterVehicleId != -1) {
    ...
    if (pVehicleList[iHelicopterVehicleId].ubMovementGroup != 0) {
      ...
      */
      ...
      BltVideoObject2(vsFB, hHandle, HELI_ICON, x, y, VO_BLT_SRCTRANSPARENCY, NULL);
...
void DisplayDestinationOfHelicopter(void) {
  ...
  if (GetLengthOfPath(pVehicleList[iHelicopterVehicleId].pMercPath) > 1) {
    ...
    GetVideoObject(&hHandle, guiHelicopterIcon);
    BltVideoObject2(vsFB, hHandle, HELI_SHADOW_ICON, x, y, VO_BLT_SRCTRANSPARENCY, NULL);
...
void BlitTownGridMarkers(void) {
  ...
  for (int iCounter = 0; iCounter < townSectors.count; iCounter++) {
    ...
    uint8_t sX = townSectors.sectors[iCounter].x;
    uint8_t sY = townSectors.sectors[iCounter].y;
    // skip Orta/Tixa until found
...
BOOLEAN DrawMilitiaPopUpBox(void) {
  ...
  BltVideoObject2(vsFB, hVObject, (uint16_t)(sSelectedMilitiaTown - 1),
...
void RenderIconsPerSectorForSelectedTown(void) {
  ...
  for (iCounter = 0; iCounter < 9; iCounter++) {
    ...
    for (iCurrentTroopIcon = 0; iCurrentTroopIcon < iTotalNumberOfTroops; iCurrentTroopIcon++) {
      ...
      BltVideoObject2(vsFB, hVObject, (uint16_t)(iCurrentIcon), sX, sY, VO_BLT_SRCTRANSPARENCY,
                      NULL);
    }
  }
...
void ShowHighLightedSectorOnMilitiaMap(void) {
  ...
  if (sSectorMilitiaMapSector != -1) {
    ...
    // blt the object
    BltVideoObject2(vsFB, hVObject, 0, sX, sY, VO_BLT_SRCTRANSPARENCY, NULL);
  }
...
void DisplayUnallocatedMilitia(void) {
  ...
  for (iCurrentTroopIcon = 0; iCurrentTroopIcon < iTotalNumberOfTroops; iCurrentTroopIcon++) {
    ...
    BltVideoObject2(vsFB, hVObject, (uint16_t)(iCurrentIcon), sX, sY, VO_BLT_SRCTRANSPARENCY, NULL);
  }
}
...
void HandleShutDownOfMilitiaPanelIfPeopleOnTheCursor(int16_t sTownValue) {
  int32_t iNumberUnderControl = 0, iNumberThatCanFitInSector = 0, iCount = 0;
...
void DrawTownMilitiaForcesOnMap(void) {
  ...
  for (int iCounter = 0; iCounter < townSectors.count; iCounter++) {
    ...
    uint8_t sSectorX = townSectors.sectors[iCounter].x;
    uint8_t sSectorY = townSectors.sectors[iCounter].y;
    if (!IsSectorEnemyControlled(sSectorX, sSectorY)) {
      struct MilitiaCount milCount = GetMilitiaInSector(sSectorX, sSectorY);
