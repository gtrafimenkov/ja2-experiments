    ...
    if (sDest != sSource) {
      if (PlayersBetweenTheseSectors(
        ...
        if (!fZoomFlag) {
          ...
          BltVideoObject(vsSB, hIconHandle, (uint16_t)iCounter, iX, iY);
...
void BlitMineIcon(uint8_t sMapX, uint8_t sMapY) {
  ...
  if (fZoomFlag) {
    ...
    BltVideoObject(vsSB, hHandle, 0, sScreenX - MAP_GRID_ZOOM_X / 4,
...
void ShowSAMSitesOnStrategicMap(void) {
  ...
  for (iCounter = 0; iCounter < NUMBER_OF_SAM_SITES; iCounter++) {
    ...
    BltVideoObject(vsSB, hHandle, ubVidObjIndex, sX, sY);
...
void DrawBullseye() {
  ...
  BltVideoObject(vsSB, hHandle, 0, sX, sY);
}
