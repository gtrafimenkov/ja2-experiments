...
void DoTransitionFromPreBattleInterfaceToAutoResolve() {
  ...
  // save it
  BlitBufferToBuffer(vsIndexFB, vsSB, (uint16_t)SrcRect.iLeft, (uint16_t)SrcRect.iTop,
                     (uint16_t)SrcRect.iRight, (uint16_t)SrcRect.iBottom);

  // hide the autoresolve
  BlitBufferToBuffer(guiEXTRABUFFER, vsIndexFB, (uint16_t)SrcRect.iLeft, (uint16_t)SrcRect.iTop,
                     (uint16_t)SrcRect.iRight, (uint16_t)SrcRect.iBottom);
...
void RenderSoldierCell(SOLDIERCELL *pCell) {
  ...
  if (pCell->uiFlags & CELL_MERC) {
    ColorFillVideoSurfaceArea(vsIndexFB, pCell->xp + 36, pCell->yp + 2, pCell->xp + 44,
                              pCell->yp + 30, 0);
    BltVObjectFromIndex(vsIndexFB, gpAR->iPanelImages, MERC_PANEL, pCell->xp, pCell->yp);
    RenderSoldierCellBars(pCell);
    x = 0;
  } else {
    BltVObjectFromIndex(vsIndexFB, gpAR->iPanelImages, OTHER_PANEL, pCell->xp, pCell->yp);
    x = 6;
  }
  if (!pCell->pSoldier->bLife) {
    SetObjectHandleShade(pCell->uiVObjectID, 0);
    if (!(pCell->uiFlags & CELL_CREATURE))
      BltVObjectFromIndex(vsIndexFB, gpAR->iFaces, HUMAN_SKULL, pCell->xp + 3 + x, pCell->yp + 3);
    else
      BltVObjectFromIndex(vsIndexFB, gpAR->iFaces, CREATURE_SKULL, pCell->xp + 3 + x,
                          pCell->yp + 3);
  } else {
    if (pCell->uiFlags & CELL_HITBYATTACKER) {
      ...
      BltVObjectFromIndex(vsIndexFB, pCell->uiVObjectID, pCell->usIndex, pCell->xp + 3 + x,
                          pCell->yp + 3);
    } else {
      SetObjectHandleShade(pCell->uiVObjectID, 0);
      BltVObjectFromIndex(vsIndexFB, pCell->uiVObjectID, pCell->usIndex, pCell->xp + 3 + x,
...
void BuildInterfaceBuffer() {
  ...
  for (y = DestRect.iTop; y < DestRect.iBottom; y += 40) {
    for (x = DestRect.iLeft; x < DestRect.iRight; x += 50) {
      BltVObjectFromIndex(gpAR->iInterfaceBuffer, gpAR->iPanelImages, C_TEXTURE, x, y);
    }
  }
  // Blit the left and right edges
  for (y = DestRect.iTop; y < DestRect.iBottom; y += 40) {
    x = DestRect.iLeft;
    BltVObjectFromIndex(gpAR->iInterfaceBuffer, gpAR->iPanelImages, L_BORDER, x, y);
    x = DestRect.iRight - 3;
    BltVObjectFromIndex(gpAR->iInterfaceBuffer, gpAR->iPanelImages, R_BORDER, x, y);
  }
  // Blit the top and bottom edges
  for (x = DestRect.iLeft; x < DestRect.iRight; x += 50) {
    y = DestRect.iTop;
    BltVObjectFromIndex(gpAR->iInterfaceBuffer, gpAR->iPanelImages, T_BORDER, x, y);
    y = DestRect.iBottom - 3;
    BltVObjectFromIndex(gpAR->iInterfaceBuffer, gpAR->iPanelImages, B_BORDER, x, y);
  }
  // Blit the 4 corners
  BltVObjectFromIndex(gpAR->iInterfaceBuffer, gpAR->iPanelImages, TL_BORDER, DestRect.iLeft,
                      DestRect.iTop);
  BltVObjectFromIndex(gpAR->iInterfaceBuffer, gpAR->iPanelImages, TR_BORDER, DestRect.iRight - 10,
                      DestRect.iTop);
  BltVObjectFromIndex(gpAR->iInterfaceBuffer, gpAR->iPanelImages, BL_BORDER, DestRect.iLeft,
                      DestRect.iBottom - 9);
  BltVObjectFromIndex(gpAR->iInterfaceBuffer, gpAR->iPanelImages, BR_BORDER, DestRect.iRight - 10,
                      DestRect.iBottom - 9);
  ...
  y = 0;
  // Top
  BltVObjectFromIndex(gpAR->iInterfaceBuffer, gpAR->iPanelImages, TOP_MIDDLE, x, y);
  // Middle
  for (y = 40; y < gpAR->sHeight - 40; y += 40) {
    BltVObjectFromIndex(gpAR->iInterfaceBuffer, gpAR->iPanelImages, AUTO_MIDDLE, x, y);
  }
  y = gpAR->sHeight - 40;
  BltVObjectFromIndex(gpAR->iInterfaceBuffer, gpAR->iPanelImages, BOT_MIDDLE, x, y);
...
void ExpandWindow() {
  ...
  if (!gpAR->ExRect.iLeft && !gpAR->ExRect.iRight) {  // First time
    ...
    // left
    BlitBufferToBuffer(vsSB, vsIndexFB, (uint16_t)gpAR->ExRect.iLeft, (uint16_t)gpAR->ExRect.iTop,
                       1, (uint16_t)(gpAR->ExRect.iBottom - gpAR->ExRect.iTop + 1));
    InvalidateRegion(gpAR->ExRect.iLeft, gpAR->ExRect.iTop, gpAR->ExRect.iLeft + 1,
                     gpAR->ExRect.iBottom + 1);
    // right
    BlitBufferToBuffer(vsSB, vsIndexFB, (uint16_t)gpAR->ExRect.iRight, (uint16_t)gpAR->ExRect.iTop,
                       1, (uint16_t)(gpAR->ExRect.iBottom - gpAR->ExRect.iTop + 1));
    ...
    InvalidateRegion(gpAR->ExRect.iLeft, gpAR->ExRect.iTop, gpAR->ExRect.iRight + 1,
                     gpAR->ExRect.iTop + 1);
    // bottom
    BlitBufferToBuffer(vsSB, vsIndexFB, (uint16_t)gpAR->ExRect.iLeft,
...
void RenderAutoResolve() {
  ...
  if (gpAR->ubBattleStatus != BATTLE_IN_PROGRESS) {
    ...
    BltVObjectFromIndex(vsIndexFB, gpAR->iIndent, 0, xp, yp);
...
void CreateAutoResolveInterface() {
  ...
  BlitBufferToBuffer(vsSB, vsIndexFB, 0, 0, 640, 480);
