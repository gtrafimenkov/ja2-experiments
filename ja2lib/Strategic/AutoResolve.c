...
void BuildInterfaceBuffer() {
  ...
  if (!AddStandardVideoSurface(&vs_desc, &gpAR->iInterfaceBuffer))
...
void CreateAutoResolveInterface() {
  ...
  if (!AddVObjectAndSetTransparency(&VObjectDesc, &gpAR->iPanelImages)) {
  ...
  if (!AddVObjectAndSetTransparency(&VObjectDesc, &gpAR->iFaces)) {
  ...
  if (!AddVObjectAndSetTransparency(&VObjectDesc, &gpAR->iIndent)) {
  ...
  for (i = 0; i < gpAR->ubMercs; i++) {
    ...
    if (!AddVObjectAndSetTransparency(&VObjectDesc, &gpMercs[i].uiVObjectID)) {
