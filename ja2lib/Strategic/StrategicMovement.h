...
struct GROUP {
  ...
  uint8_t ubRestAtFatigueLevel;  // when the group's fatigue level <= this level, they will rest upon
                               // arrival at next sector.
  uint8_t ubRestToFatigueLevel;  // when resting, the group will rest until the fatigue level reaches
                               // this level.
  uint32_t uiArrivalTime;  // the arrival time in world minutes that the group will arrive at the next
                         // sector.
  ...
  uint8_t ubCreatedSectorID;  // used for debugging strategic AI for keeping track of the sector ID a
