  ...
  */
  {
    ...
    CHECKF(AddStandardVideoObject(&VObjectDesc, &gusRadarImage));
...
void RenderRadarScreen() {
  ...
  if (fInterfacePanelDirty == DIRTYLEVEL2 && fImageLoaded) {
    ...
    BltVObjectFromIndex(vsSB, gusRadarImage, 0, RADAR_WINDOW_X, gsRadarY, VO_BLT_SRCTRANSPARENCY,
                        NULL);
  }
...
BOOLEAN CreateDestroyMouseRegionsForSquadList(void) {
  ...
  if ((fRenderRadarScreen == FALSE) && (fCreated == FALSE)) {
    ...
    CHECKF(AddStandardVideoObject(&VObjectDesc, &uiHandle));

    GetVideoObject(&hHandle, uiHandle);
    BltVideoObject(vsSB, hHandle, 0, 538, 0 + gsVIEWPORT_END_Y, VO_BLT_SRCTRANSPARENCY, NULL);
