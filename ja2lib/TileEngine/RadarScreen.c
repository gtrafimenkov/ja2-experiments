  ...
  */
  {
    ...
    CHECKF(AddVObject(CreateVObjectFromFile(ImageFile), &gusRadarImage));
...
void RenderRadarScreen() {
  ...
  if (fInterfacePanelDirty == DIRTYLEVEL2 && fImageLoaded) {
    ...
    BltVObjectFromIndex(vsSaveBuffer, gusRadarImage, 0, RADAR_WINDOW_X, gsRadarY);
  }
...
BOOLEAN CreateDestroyMouseRegionsForSquadList(void) {
  ...
  if ((fRenderRadarScreen == FALSE) && (fCreated == FALSE)) {
    // create regions
    // load graphics
    CHECKF(AddVObject(CreateVObjectFromFile("INTERFACE\\squadpanel.sti"), &uiHandle));

    GetVideoObject(&hHandle, uiHandle);
    BltVideoObject(vsSaveBuffer, hHandle, 0, 538, 0 + gsVIEWPORT_END_Y);
