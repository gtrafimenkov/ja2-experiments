...
void RenderTacticalPlacementGUI() {
  ...
  if (gfTacticalPlacementGUIDirty) {
    BltVObjectFromIndex(vsIndexFB, giOverheadPanelImage, 0, 0, 320);
    ...
    for (i = 0; i < giPlacements; i++) {  // Render the mercs
      ...
      BltVObjectFromIndex(vsIndexFB, giMercPanelImage, 0, xp, yp);
      BltVObjectFromIndex(vsIndexFB, gMercPlacement[i].uiVObjectID, 0, xp + 2, yp + 2);
      // HEALTH BAR
    ...
    BlitBufferToBuffer(vsIndexFB, vsSaveBufferID, 0, 320, 640, 160);
  }
  if (gfValidLocationsChanged) {
    ...
    BlitBufferToBuffer(vsSaveBufferID, vsIndexFB, 4, 4, 636, 320);
