...
BOOLEAN InitializeTacticalInterface() {
  ...
  if (!AddVObjectAndSetTransparency(&VObjectDesc, &guiGUNSM))
  ...
  if (!AddVObjectAndSetTransparency(&VObjectDesc, &guiRADIO))
...
void CreateTopMessage(uint32_t uiSurface, uint8_t ubType, wchar_t *psString) {
  ...
  if (!AddVObjectAndSetTransparency(&VObjectDesc, &uiBAR))
