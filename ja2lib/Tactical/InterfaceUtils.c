...
BOOLEAN LoadCarPortraitValues(void) {
  ...
  for (iCounter = 0; iCounter < NUMBER_CAR_PORTRAITS; iCounter++) {
    ...
    strcopy(ImageFile, sizeof(ImageFile), pbCarPortraitFileNames[iCounter]);
    CHECKF(AddVObject(CreateVObjectFromFile(ImageFile), &giCarPortraits[iCounter]));
  }
  return (TRUE);
}
...
void DrawLifeUIBarEx(struct SOLDIERTYPE *pSoldier, int16_t sXPos, int16_t sYPos, int16_t sWidth,
  ...
  usLineColor = rgb32_to_rgb16(LIFE_BAR_SHADOW);
  RectangleDraw(TRUE, sXPos, (int32_t)dStart, sXPos, (int32_t)(dStart - dEnd), usLineColor,
                pDestBuf);

  usLineColor = rgb32_to_rgb16(LIFE_BAR);
  ...
  if (bBandage) {
    ...
    usLineColor = rgb32_to_rgb16(BANDAGE_BAR_SHADOW);
    RectangleDraw(TRUE, sXPos, (int32_t)dStart, sXPos, (int32_t)(dStart - dEnd), usLineColor,
                  pDestBuf);

    usLineColor = rgb32_to_rgb16(BANDAGE_BAR);
  ...
  if (pSoldier->bBleeding) {
    ...
    usLineColor = rgb32_to_rgb16(BLEEDING_BAR_SHADOW);
    RectangleDraw(TRUE, sXPos, (int32_t)dStart, sXPos, (int32_t)(dStart - dEnd), usLineColor,
                  pDestBuf);

    usLineColor = rgb32_to_rgb16(BLEEDING_BAR);
    RectangleDraw(TRUE, sXPos + 1, (int32_t)dStart, sXPos + 1, (int32_t)(dStart - dEnd),
                  usLineColor, pDestBuf);
...
void DrawBreathUIBarEx(struct SOLDIERTYPE *pSoldier, int16_t sXPos, int16_t sYPos, int16_t sWidth,
  ...
  if (pSoldier->bBreathMax <= 97) {
    ...
    usLineColor = rgb32_to_rgb16(BREATH_BAR_SHAD_BACK);
    RectangleDraw(TRUE, sXPos, (int32_t)dStart, sXPos, (int32_t)(dStart - dEnd), usLineColor,
                  pDestBuf);
  ...
  usLineColor = rgb32_to_rgb16(CURR_MAX_BREATH_SHADOW);
  RectangleDraw(TRUE, sXPos, (int32_t)dStart, sXPos, (int32_t)(dStart - dEnd), usLineColor,
                pDestBuf);

  usLineColor = rgb32_to_rgb16(CURR_MAX_BREATH);
  RectangleDraw(TRUE, sXPos + 1, (int32_t)dStart, sXPos + 1, (int32_t)(dStart - dEnd), usLineColor,
                pDestBuf);
  ...
  usLineColor = rgb32_to_rgb16(CURR_BREATH_BAR_SHADOW);
  RectangleDraw(TRUE, sXPos, (int32_t)dStart, sXPos, (int32_t)(dStart - dEnd), usLineColor,
                pDestBuf);

  usLineColor = rgb32_to_rgb16(CURR_BREATH_BAR);
  RectangleDraw(TRUE, sXPos + 1, (int32_t)dStart, sXPos + 1, (int32_t)(dStart - dEnd), usLineColor,
                pDestBuf);
...
void DrawMoraleUIBarEx(struct SOLDIERTYPE *pSoldier, int16_t sXPos, int16_t sYPos, int16_t sWidth,
  ...
  usLineColor = rgb32_to_rgb16(MORALE_BAR_SHADOW);
  RectangleDraw(TRUE, sXPos, (int32_t)dStart, sXPos, (int32_t)(dStart - dEnd), usLineColor,
                pDestBuf);

  usLineColor = rgb32_to_rgb16(MORALE_BAR);
  RectangleDraw(TRUE, sXPos + 1, (int32_t)dStart, sXPos + 1, (int32_t)(dStart - dEnd), usLineColor,
                pDestBuf);
...
void DrawItemUIBarEx(struct OBJECTTYPE *pObject, uint8_t ubStatus, int16_t sXPos, int16_t sYPos,
  ...
  // usLineColor = rgb32_to_rgb16( STATUS_BAR );
  usLineColor = sColor1;
  RectangleDraw(TRUE, sXPos, (int32_t)dStart, sXPos, (int32_t)(dStart - dEnd), usLineColor,
                pDestBuf);
...
void RenderSoldierFace(struct SOLDIERTYPE *pSoldier, int16_t sFaceX, int16_t sFaceY,
  ...
  if (IsSolActive(pSoldier)) {
    if (pSoldier->uiStatusFlags & SOLDIER_VEHICLE) {
      ...
      BltVObjectFromIndex(vsSaveBuffer, giCarPortraits[ubVehicleType], 0, sFaceX, sFaceY);
    ...
    if (fAutoFace) {
      ...
      if (gFacesData[pSoldier->iFaceIndex].uiFlags & FACE_INACTIVE_HANDLED_ELSEWHERE) {
        ...
        SetAutoFaceActiveFromSoldier(vsFB, vsSaveBuffer, GetSolID(pSoldier), sFaceX, sFaceY);
      }
    }
    ...
    BltVObjectFromIndex(vsSaveBuffer, guiCLOSE, 5, sFaceX, sFaceY);
    RestoreExternBackgroundRect(sFaceX, sFaceY, FACE_WIDTH, FACE_HEIGHT);
  }
}
