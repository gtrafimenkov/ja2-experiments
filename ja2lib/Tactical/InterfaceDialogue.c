...
BOOLEAN InternalInitTalkingMenu(uint8_t ubCharacterNum, int16_t sX, int16_t sY) {
  ...
  // Load
  if (AddStandardVideoObject(&VObjectDesc, &(gTalkPanel.uiPanelVO)) == FALSE) {
    return (0);
  }
...
void RenderTalkingMenu() {
  ...
  if (gTalkPanel.fDirtyLevel == DIRTYLEVEL2) {
    ...
    // Render box!
    BltVObjectFromIndexOld(vsIndexFB, gTalkPanel.uiPanelVO, 0, gTalkPanel.sX, gTalkPanel.sY);
    ...
    if (pFace->fTalking || !DialogueQueueIsEmpty()) {
      ShadowVideoSurfaceRect(
          vsIndexFB, (int16_t)(gTalkPanel.sX + TALK_PANEL_SHADOW_AREA_X),
