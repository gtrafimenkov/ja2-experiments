...
BOOLEAN InitInvSlotInterface(INV_REGION_DESC *pRegionDesc, INV_REGION_DESC *pCamoRegion,
  ...
  CHECKF(AddStandardVideoObject(&VObjectDesc, &(guiBodyInvVO[1][0])));

  VObjectDesc.fCreateFlags = VOBJECT_CREATE_FROMFILE;
  FilenameForBPP("INTERFACE\\inventory_figure_large_male_H.sti", VObjectDesc.ImageFile);
  CHECKF(AddStandardVideoObject(&VObjectDesc, &(guiBodyInvVO[1][1])));

  VObjectDesc.fCreateFlags = VOBJECT_CREATE_FROMFILE;
  FilenameForBPP("INTERFACE\\inventory_normal_male.sti", VObjectDesc.ImageFile);
  CHECKF(AddStandardVideoObject(&VObjectDesc, &(guiBodyInvVO[0][0])));

  VObjectDesc.fCreateFlags = VOBJECT_CREATE_FROMFILE;
  FilenameForBPP("INTERFACE\\inventory_normal_male_H.sti", VObjectDesc.ImageFile);
  CHECKF(AddStandardVideoObject(&VObjectDesc, &(guiBodyInvVO[0][1])));

  VObjectDesc.fCreateFlags = VOBJECT_CREATE_FROMFILE;
  FilenameForBPP("INTERFACE\\inventory_normal_male.sti", VObjectDesc.ImageFile);
  CHECKF(AddStandardVideoObject(&VObjectDesc, &(guiBodyInvVO[2][0])));

  VObjectDesc.fCreateFlags = VOBJECT_CREATE_FROMFILE;
  FilenameForBPP("INTERFACE\\inventory_normal_male.sti", VObjectDesc.ImageFile);
  CHECKF(AddStandardVideoObject(&VObjectDesc, &(guiBodyInvVO[2][1])));

  VObjectDesc.fCreateFlags = VOBJECT_CREATE_FROMFILE;
  FilenameForBPP("INTERFACE\\inventory_figure_female.sti", VObjectDesc.ImageFile);
  CHECKF(AddStandardVideoObject(&VObjectDesc, &(guiBodyInvVO[3][0])));

  VObjectDesc.fCreateFlags = VOBJECT_CREATE_FROMFILE;
  FilenameForBPP("INTERFACE\\inventory_figure_female_H.sti", VObjectDesc.ImageFile);
  CHECKF(AddStandardVideoObject(&VObjectDesc, &(guiBodyInvVO[3][1])));

  // add gold key graphic
  VObjectDesc.fCreateFlags = VOBJECT_CREATE_FROMFILE;
  FilenameForBPP("INTERFACE\\gold_key_button.sti", VObjectDesc.ImageFile);
  CHECKF(AddStandardVideoObject(&VObjectDesc, &guiGoldKeyVO));
...
void RenderInvBodyPanel(struct SOLDIERTYPE *pSoldier, int16_t sX, int16_t sY) {
  ...
  BltVObjectFromIndex(vsSaveBufferID, guiBodyInvVO[pSoldier->ubBodyType][bSubImageIndex], 0, sX,
                      sY);
}
  ...
  if (InItemDescriptionBox() || InItemStackPopup() || InKeyRingPopup()) {
    ...
    if (KeyExistsInKeyRing(pSoldier, ANYKEY, NULL)) {
      ...
      if (guiCurrentItemDescriptionScreen != MAP_SCREEN) {
        BltVObjectFromIndex(vsSaveBufferID, guiGoldKeyVO, 0, 496, 446);
        RestoreExternBackgroundRect(496, 446, 29, 23);
      } else {
        BltVObjectFromIndex(vsSaveBufferID, guiGoldKeyVO, 0, 217, 271);
...
void INVRenderINVPanelItem(struct SOLDIERTYPE *pSoldier, int16_t sPocket, uint8_t fDirtyLevel) {
  ...
  if (fDirtyLevel == DIRTYLEVEL2) {
    ...
    if (sPocket == SECONDHANDPOS && Item[pSoldier->inv[HANDPOS].usItem].fFlags & ITEM_TWO_HANDED) {
      if (guiCurrentItemDescriptionScreen != MAP_SCREEN) {
        BltVObjectFromIndex(vsSaveBufferID, guiSecItemHiddenVO, 0, 217, 448);
        RestoreExternBackgroundRect(217, 448, 72, 28);
      } else {
        BltVObjectFromIndex(vsSaveBufferID, guiMapInvSecondHandBlockout, 0, 14, 218);
  ...
  // INVRenderItem( vsSaveBufferID, pObject, (int16_t)(sX + gSMInvData[ sPocket ].sSubX),
  ...
  INVRenderItem(vsSaveBufferID, pSoldier, pObject, sX, sY, gSMInvData[sPocket].sWidth,
...
void HandleNewlyAddedItems(struct SOLDIERTYPE *pSoldier, BOOLEAN *fDirtyLevel) {
  ...
  for (cnt = 0; cnt < NUM_INV_SLOTS; cnt++) {
    ...
    if (pSoldier->bNewItemCount[cnt] > 0) {
      ...
      INVRenderItem(vsSaveBufferID, pSoldier, pObject, sX, sY, gSMInvData[cnt].sWidth,
...
BOOLEAN InternalInitItemDescriptionBox(struct OBJECTTYPE *pObject, int16_t sX, int16_t sY,
  ...
  CHECKF(AddStandardVideoObject(&VObjectDesc, &guiItemDescBox));

  VObjectDesc.fCreateFlags = VOBJECT_CREATE_FROMFILE;
  strcpy(VObjectDesc.ImageFile, "INTERFACE\\iteminfoc.STI");
  CHECKF(AddStandardVideoObject(&VObjectDesc, &guiMapItemDescBox));

  VObjectDesc.fCreateFlags = VOBJECT_CREATE_FROMFILE;
  strcpy(VObjectDesc.ImageFile, "INTERFACE\\bullet.STI");
  CHECKF(AddStandardVideoObject(&VObjectDesc, &guiBullet));
  ...
  if (gpItemDescObject->usItem != MONEY) {
    ...
    CHECKF(AddStandardVideoObject(&VObjectDesc, &guiMoneyGraphicsForDescBox));
...
void RenderItemDescriptionBox() {
  ...
  if ((guiCurrentItemDescriptionScreen == MAP_SCREEN) && (gfInItemDescBox)) {
    ...
    BltVObjectFromIndex(vsSaveBufferID, guiMapItemDescBox, 0, gsInvDescX, gsInvDescY);

    // Display the money 'seperating' border
    if (gpItemDescObject->usItem == MONEY) {
      // Render the money Boxes
      BltVObjectFromIndex(vsSaveBufferID, guiMoneyGraphicsForDescBox, 0,
                          (uint16_t)(gMapMoneyButtonLoc.x + gMoneyButtonOffsets[0].x),
                          (uint16_t)(gMapMoneyButtonLoc.y + gMoneyButtonOffsets[0].y));
    }

    // Display item
    BltVideoObjectOutlineShadowFromIndex(vsSaveBufferID, guiItemGraphic, 0, sCenX - 2, sCenY + 2);

    BltVObjectFromIndex(vsSaveBufferID, guiItemGraphic, 0, sCenX, sCenY);
    ...
    for (cnt = 0; cnt < MAX_ATTACHMENTS; cnt++) {
      if (gpItemDescObject->usAttachItem[cnt] != NOTHING) {
        if (guiCurrentItemDescriptionScreen == MAP_SCREEN) {
          ...
          INVRenderItem(vsSaveBufferID, NULL, gpItemDescObject, sCenX, sCenY,
                        gMapItemDescAttachmentsXY[cnt].sWidth,
    ...
    if (Item[gpItemDescObject->usItem].usItemClass & IC_GUN) {
      ...
      BltVObjectFromIndex(vsSaveBufferID, guiBullet, 0, MAP_BULLET_SING_X, MAP_BULLET_SING_Y);

      if (Weapon[gpItemDescObject->usItem].ubShotsPerBurst > 0) {
        for (cnt = 0; cnt < Weapon[gpItemDescObject->usItem].ubShotsPerBurst; cnt++) {
          BltVObjectFromIndex(vsSaveBufferID, guiBullet, 0,
...
// Render name
    ...
    */
    ...
    BltVObjectFromIndex(vsSaveBufferID, guiItemDescBox, 0, gsInvDescX, gsInvDescY);
    ...
    if (Item[gpItemDescObject->usItem].usItemClass & IC_GUN) {
      // display bullets for ROF
      BltVObjectFromIndex(vsSaveBufferID, guiBullet, 0, BULLET_SING_X, BULLET_SING_Y);
...
BOOLEAN InitItemStackPopup(struct SOLDIERTYPE *pSoldier, uint8_t ubPosition, int16_t sInvX,
  ...
  CHECKF(AddStandardVideoObject(&VObjectDesc, &guiItemPopupBoxes));
...
void RenderItemStackPopup(BOOLEAN fFullRender) {
  ...
  if (gfInItemStackPopup) {
    ...
    // Shadow Area
    if (fFullRender) {
      ShadowVideoSurfaceRect(vsIndexFB, gsItemPopupInvX, gsItemPopupInvY,
  ...
  for (cnt = 0; cnt < gubNumItemPopups; cnt++) {
    BltVObjectFromIndex(vsIndexFB, guiItemPopupBoxes, 0, gsItemPopupX + (cnt * usWidth),
                        gsItemPopupY);
    ...
    if (cnt < gpItemPopupObject->ubNumberOfObjects) {
      ...
      INVRenderItem(vsIndexFB, NULL, gpItemPopupObject, sX, sY, 29, 23, DIRTYLEVEL2, NULL,
...
void RenderKeyRingPopup(BOOLEAN fFullRender) {
  ...
  if (gfInKeyRingPopup) {
    ...
    // Shadow Area
    if (fFullRender) {
      ShadowVideoSurfaceRect(vsIndexFB, 0, gsKeyRingPopupInvY,
...
BOOLEAN LoadTileGraphicForItem(INVTYPE *pItem, uint32_t *puiVo) {
  ...
  CHECKF(AddStandardVideoObject(&VObjectDesc, &uiVo));
...
BOOLEAN InitializeItemPickupMenu(struct SOLDIERTYPE *pSoldier, int16_t sGridNo,
  ...
  CHECKF(AddStandardVideoObject(&VObjectDesc, &(gItemPickupMenu.uiPanelVo)));
...
void RenderItemPickupMenu() {
  ...
  if (gItemPickupMenu.fDirtyLevel == DIRTYLEVEL2) {
    ...
    for (cnt = 0; cnt < gItemPickupMenu.bNumSlotsPerPage; cnt++) {
      ...
      BltVObjectFromIndex(vsIndexFB, gItemPickupMenu.uiPanelVo, usSubRegion, sX, sY);
    ...
    if (gItemPickupMenu.bNumSlotsPerPage == NUM_PICKUP_SLOTS &&
        gItemPickupMenu.ubTotalItems > NUM_PICKUP_SLOTS) {
      BltVObjectFromIndex(vsIndexFB, gItemPickupMenu.uiPanelVo, 2, sX, sY);
    } else {
      BltVObjectFromIndex(vsIndexFB, gItemPickupMenu.uiPanelVo, 3, sX, sY);
    }
