...
BOOLEAN LoadFiles(void) {
  ...
  CHECKF(AddVObjectAndSetTransparency(&VObjectDesc, &guiTITLE));

  // top portion of the screen background
  VObjectDesc.fCreateFlags = VOBJECT_CREATE_FROMFILE;
  FilenameForBPP("LAPTOP\\fileviewer.sti", VObjectDesc.ImageFile);
  CHECKF(AddVObjectAndSetTransparency(&VObjectDesc, &guiTOP));

  // the highlight
  VObjectDesc.fCreateFlags = VOBJECT_CREATE_FROMFILE;
  FilenameForBPP("LAPTOP\\highlight.sti", VObjectDesc.ImageFile);
  CHECKF(AddVObjectAndSetTransparency(&VObjectDesc, &guiHIGHLIGHT));

  // top portion of the screen background
  VObjectDesc.fCreateFlags = VOBJECT_CREATE_FROMFILE;
  FilenameForBPP("LAPTOP\\fileviewerwhite.sti", VObjectDesc.ImageFile);
  CHECKF(AddVObjectAndSetTransparency(&VObjectDesc, &guiFileBack));
...
BOOLEAN DisplayFormattedText(void) {
  ...
  switch (pFilesList->ubFormat) {
    ...
    case 1:
      ...
      CHECKF(AddVObjectAndSetTransparency(&VObjectDesc, &uiFirstTempPicture));
    ...
    case 2:
      ...
      CHECKF(AddVObjectAndSetTransparency(&VObjectDesc, &uiSecondTempPicture));
...
BOOLEAN HandleSpecialFiles(uint8_t ubFormat) {
  ...
  if (giFilesPage == 0) {
    ...
    CHECKF(AddVObjectAndSetTransparency(&VObjectDesc, &uiPicture));
