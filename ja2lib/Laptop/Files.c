...
BOOLEAN LoadFiles(void) {
  ...
  CHECKF(AddStandardVideoObject(&VObjectDesc, &guiTOP));

  // the highlight
  VObjectDesc.fCreateFlags = VOBJECT_CREATE_FROMFILE;
  FilenameForBPP("LAPTOP\\highlight.sti", VObjectDesc.ImageFile);
  CHECKF(AddStandardVideoObject(&VObjectDesc, &guiHIGHLIGHT));

  // top portion of the screen background
  VObjectDesc.fCreateFlags = VOBJECT_CREATE_FROMFILE;
  FilenameForBPP("LAPTOP\\fileviewerwhite.sti", VObjectDesc.ImageFile);
  CHECKF(AddStandardVideoObject(&VObjectDesc, &guiFileBack));
...
void DisplayFilesList(void) {
  ...
  while ((pFilesList))  //&&(iCounter < MAX_FILES_LIST_LENGTH))
  {
    if (iCounter == iHighLightFileLine) {
      ...
      BltVideoObject(vsIndexFB, hHandle, 0, FILES_SENDER_TEXT_X - 5,
...
BOOLEAN DisplayFormattedText(void) {
  ...
  BltVideoObject(vsIndexFB, hHandle, 0, FILE_VIEWER_X, FILE_VIEWER_Y - 4);
  ...
  switch (pFilesList->ubFormat) {
    ...
    case 1:
      ...
      CHECKF(AddStandardVideoObject(&VObjectDesc, &uiFirstTempPicture));
    ...
    case 2:
      ...
      CHECKF(AddStandardVideoObject(&VObjectDesc, &uiSecondTempPicture));
      ...
      // blt background to screen
      BltVideoObject(vsIndexFB, hHandle, 0, FILE_VIEWER_X + usFreeSpace, FILE_VIEWER_Y + 10);
...
BOOLEAN HandleSpecialFiles(uint8_t ubFormat) {
  ...
  if (giFilesPage == 0) {
    ...
    CHECKF(AddStandardVideoObject(&VObjectDesc, &uiPicture));

    // get title bar object
    GetVideoObject(&hHandle, uiPicture);

    // blt title bar to screen
    BltVideoObject(vsIndexFB, hHandle, 0, 300, 270);
    ...
    // blt title bar to screen
    BltVideoObject(vsIndexFB, hHandle, 0, 260, 225);
    ...
    // blt title bar to screen
    BltVideoObject(vsIndexFB, hHandle, 0, 260, 85);
...
BOOLEAN HandleSpecialTerroristFile(int32_t iFileNumber, char *sPictureName) {
  ...
  while (pTempString) {
    ...
    if ((giFilesPage == 0) && (iCounter == 5)) {
      ...
      BltVideoObject(vsIndexFB, hHandle, 0, (int16_t)(FILE_VIEWER_X + 30),
      ...
      BltVideoObject(vsIndexFB, hHandle, 0, (int16_t)(FILE_VIEWER_X + 25),
