...
BOOLEAN EnterBobbyRGuns() {
  ...
  CHECKF(AddStandardVideoObject(&VObjectDesc, &guiGunBackground));

  // load the gunsgrid graphic and add it
  VObjectDesc.fCreateFlags = VOBJECT_CREATE_FROMFILE;
  FilenameForBPP("LAPTOP\\gunsgrid.sti", VObjectDesc.ImageFile);
  CHECKF(AddStandardVideoObject(&VObjectDesc, &guiGunsGrid));
...
void RenderBobbyRGuns() {
  ...
  BltVideoObjectOld(vsIndexFB, hPixHandle, 0, BOBBYR_GRIDLOC_X, BOBBYR_GRIDLOC_Y);
...
BOOLEAN DisplayBobbyRBrTitle() {
  ...
  BltVideoObjectOld(vsIndexFB, hPixHandle, 0, BOBBYR_BRTITLE_X, BOBBYR_BRTITLE_Y);
...
BOOLEAN InitBobbyBrTitle() {
  ...
  CHECKF(AddStandardVideoObject(&VObjectDesc, &guiBrTitle));
...
BOOLEAN DisplayBigItemImage(uint16_t usIndex, uint16_t PosY) {
  ...
  BltVideoObjectOld(vsIndexFB, hPixHandle, 0, sCenX, sCenY);
