...
void RenderAIM() {
  ...
  BltVideoObject(vsFB, hMemberCardHandle, 0, MEMBERCARD_X, MEMBERCARD_Y, VO_BLT_SRCTRANSPARENCY,
                 NULL);

  // Policies
  GetVideoObject(&hPoliciesHandle, guiPolicies);
  BltVideoObject(vsFB, hPoliciesHandle, 0, POLICIES_X, POLICIES_Y, VO_BLT_SRCTRANSPARENCY, NULL);

  // Links
  GetVideoObject(&hLinksHandle, guiLinks);
  BltVideoObject(vsFB, hLinksHandle, 0, LINKS_X, LINKS_Y, VO_BLT_SRCTRANSPARENCY, NULL);

  // History
  GetVideoObject(&hHistoryHandle, guiHistory);
  BltVideoObject(vsFB, hHistoryHandle, 0, HISTORY_X, HISTORY_Y, VO_BLT_SRCTRANSPARENCY, NULL);
...
BOOLEAN InitAimDefaults() {
  ...
  CHECKF(AddStandardVideoObject(&VObjectDesc, &guiRustBackGround));

  // load the Aim Symbol graphic and add it
  VObjectDesc.fCreateFlags = VOBJECT_CREATE_FROMFILE;
  GetMLGFilename(VObjectDesc.ImageFile, MLG_AIMSYMBOL);
  CHECKF(AddStandardVideoObject(&VObjectDesc, &guiAimSymbol));
...
BOOLEAN DrawAimDefaults() {
  ...
  for (y = 0; y < 4; y++) {
    ...
    for (x = 0; x < 4; x++) {
      BltVideoObject(vsFB, hRustBackGroundHandle, 0, uiPosX, uiPosY, VO_BLT_SRCTRANSPARENCY, NULL);
  ...
  BltVideoObject(vsFB, hAimSymbolHandle, 0, AIM_SYMBOL_X, AIM_SYMBOL_Y, VO_BLT_SRCTRANSPARENCY,
                 NULL);
  ...
  if (((uiCurTime - uiLastTime) > AIM_FLOWER_AD_DELAY) || fRedraw) {
    ...
    if (ubSubImage == AIM_FLOWER_NUM_SUBIMAGES) {
      if (ubCount == 0 || fRedraw) {
        ...
        BltVideoObject(vsFB, hAdHandle, 0, WARNING_X, WARNING_Y, VO_BLT_SRCTRANSPARENCY, NULL);
      ...
      GetVideoObject(&hAdHandle, guiFlowerAdvertisement);
      BltVideoObject(vsFB, hAdHandle, ubSubImage, WARNING_X, WARNING_Y, VO_BLT_SRCTRANSPARENCY,
                     NULL);
...
BOOLEAN DrawWarningBox(BOOLEAN fInit, BOOLEAN fRedraw) {
  ...
  if (fInit || fRedraw) {
    ...
    BltVideoObject(vsFB, hWarningHandle, 0, WARNING_X, WARNING_Y, VO_BLT_SRCTRANSPARENCY, NULL);
...
BOOLEAN DisplayBobbyRAd(BOOLEAN fInit, BOOLEAN fRedraw) {
  ...
  /*
                  ...
                  {
                          if( ubDuckImage >= AIM_AD_BOBBYR_AD_NUM_DUCK_SUBIMAGES )
                                  BltVideoObject(vsFB, hAdHandle,
     (uint16_t)(ubDuckImage-AIM_AD_BOBBYR_AD_NUM_DUCK_SUBIMAGES), WARNING_X, WARNING_Y,
     VO_BLT_SRCTRANSPARENCY,NULL); else BltVideoObject(vsFB, hAdHandle,
     ubDuckImage,WARNING_X, WARNING_Y, VO_BLT_SRCTRANSPARENCY,NULL);
                  ...
                  if( ubSubImage == 5 )
                  {
                          ...
                          {
                                  //Blit the ad
                                  BltVideoObject(vsFB, hAdHandle, ubSubImage,WARNING_X,
