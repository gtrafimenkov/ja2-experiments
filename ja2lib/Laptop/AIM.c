...
void RenderAIM() {
  ...
  BltVideoObjectOld(vsIndexFB, hMemberCardHandle, 0, MEMBERCARD_X, MEMBERCARD_Y);

  // Policies
  GetVideoObject(&hPoliciesHandle, guiPolicies);
  BltVideoObjectOld(vsIndexFB, hPoliciesHandle, 0, POLICIES_X, POLICIES_Y);

  // Links
  GetVideoObject(&hLinksHandle, guiLinks);
  BltVideoObjectOld(vsIndexFB, hLinksHandle, 0, LINKS_X, LINKS_Y);

  // History
  GetVideoObject(&hHistoryHandle, guiHistory);
  BltVideoObjectOld(vsIndexFB, hHistoryHandle, 0, HISTORY_X, HISTORY_Y);
...
BOOLEAN DrawAimDefaults() {
  ...
  for (y = 0; y < 4; y++) {
    ...
    for (x = 0; x < 4; x++) {
      BltVideoObjectOld(vsIndexFB, hRustBackGroundHandle, 0, uiPosX, uiPosY);
  ...
  BltVideoObjectOld(vsIndexFB, hAimSymbolHandle, 0, AIM_SYMBOL_X, AIM_SYMBOL_Y);
  ...
  if (((uiCurTime - uiLastTime) > AIM_FLOWER_AD_DELAY) || fRedraw) {
    ...
    if (ubSubImage == AIM_FLOWER_NUM_SUBIMAGES) {
      if (ubCount == 0 || fRedraw) {
        ...
        BltVideoObjectOld(vsIndexFB, hAdHandle, 0, WARNING_X, WARNING_Y);
      ...
      GetVideoObject(&hAdHandle, guiFlowerAdvertisement);
      BltVideoObjectOld(vsIndexFB, hAdHandle, ubSubImage, WARNING_X, WARNING_Y);
...
BOOLEAN DrawWarningBox(BOOLEAN fInit, BOOLEAN fRedraw) {
  ...
  if (fInit || fRedraw) {
    ...
    BltVideoObjectOld(vsIndexFB, hWarningHandle, 0, WARNING_X, WARNING_Y);
