...
BOOLEAN LoadHistory(void) {
  ...
  CHECKF(AddVObjectAndSetTransparency(&VObjectDesc, &guiSHADELINE));
  ...
  Not being used???  DF commented out
    ...
    VObjectDesc.fCreateFlags=VOBJECT_CREATE_FROMFILE;
          ...
          CHECKF(AddVObjectAndSetTransparency(&VObjectDesc, &guiVERTLINE));
  */
  // black divider line - long ( 480 length)
  VObjectDesc.fCreateFlags = VOBJECT_CREATE_FROMFILE;
  FilenameForBPP("LAPTOP\\divisionline480.sti", VObjectDesc.ImageFile);
  CHECKF(AddVObjectAndSetTransparency(&VObjectDesc, &guiLONGLINE));
