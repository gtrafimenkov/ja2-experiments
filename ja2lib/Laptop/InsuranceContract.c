...
BOOLEAN EnterInsuranceContract() {
  ...
  CHECKF(AddStandardVideoObject(&VObjectDesc, &guiInsOrderGridImage));

  // load the Insurance bullet graphic and add it
  VObjectDesc.fCreateFlags = VOBJECT_CREATE_FROMFILE;
  FilenameForBPP("LAPTOP\\bullet.sti", VObjectDesc.ImageFile);
  CHECKF(AddStandardVideoObject(&VObjectDesc, &guiInsOrderBulletImage));
...
void RenderInsuranceContract() {
  ...
  BltVideoObject(vsIndexFB, hPixHandle, 0, INS_CTRCT_FIRST_BULLET_TEXT_X,
...
BOOLEAN DisplayOrderGrid(uint8_t ubGridNumber, uint8_t ubMercID) {
  ...
  BltVideoObject(vsIndexFB, hPixHandle, 0, usPosX, INS_CTRCT_ORDER_GRID1_Y);

  // load the mercs face graphic and add it
  VObjectDesc.fCreateFlags = VOBJECT_CREATE_FROMFILE;
  sprintf(sTemp, "FACES\\%02d.sti", ubMercID);
  FilenameForBPP(sTemp, VObjectDesc.ImageFile);
  CHECKF(AddStandardVideoObject(&VObjectDesc, &uiInsMercFaceImage));
  ...
  BltVideoObject(vsIndexFB, hPixHandle, 0, usPosX + INS_CTRCT_OG_FACE_OFFSET_X,
