...
void RenderMercs() {
  ...
  BltVideoObjectOld(vsIndexFB, hPixHandle, 0, MERC_TITLE_X, MERC_TITLE_Y);

  // Speck Portrait
  GetVideoObject(&hPixHandle, guiSpecPortrait);
  BltVideoObjectOld(vsIndexFB, hPixHandle, 0, MERC_PORTRAIT_X, MERC_PORTRAIT_Y);

  // Account Box
  GetVideoObject(&hPixHandle, guiAccountBox);
  BltVideoObjectOld(vsIndexFB, hPixHandle, 0, MERC_ACCOUNT_BOX_X, MERC_ACCOUNT_BOX_Y);

  // Files Box
  GetVideoObject(&hPixHandle, guiFilesBox);
  BltVideoObjectOld(vsIndexFB, hPixHandle, 0, MERC_FILE_BOX_X, MERC_FILE_BOX_Y);
...
BOOLEAN InitMercBackGround() {
  ...
  CHECKF(AddStandardVideoObject(&VObjectDesc, &guiMercBackGround));
...
BOOLEAN HandleSpeckTalking(BOOLEAN fReset) {
  ...
  if (!BltStretchVideoSurface(vsIndexFB, guiMercVideoFaceBackground, 0, 0, VS_BLT_USECOLORKEY,
                              &SrcRect, &DestRect))
    return (FALSE);
...
BOOLEAN PixelateVideoMercImage(BOOLEAN fUp, uint16_t usPosX, uint16_t usPosY, uint16_t usWidth,
  ...
  pBuffer = (uint16_t *)LockVSurfaceByID(vsIndexFB, &uiPitch);
  Assert(pBuffer);
...
BOOLEAN DisplayMercVideoIntro(uint16_t usTimeTillFinish) {
  ...
  ColorFillVideoSurfaceArea(
      vsIndexFB, MERC_VIDEO_FACE_X, MERC_VIDEO_FACE_Y, MERC_VIDEO_FACE_X + MERC_VIDEO_FACE_WIDTH,
...
void DrawMercVideoBackGround() {
  ...
  BltVideoObjectOld(vsIndexFB, hPixHandle, 0, MERC_VIDEO_BACKGROUND_X, MERC_VIDEO_BACKGROUND_Y);
