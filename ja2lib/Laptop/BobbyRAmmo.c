...
BOOLEAN EnterBobbyRAmmo() {
  ...
  CHECKF(AddVObjectAndSetTransparency(CreateVideoObject(&VObjectDesc), &guiAmmoBackground));

  // load the gunsgrid graphic and add it
  VObjectDesc.fCreateFlags = VOBJECT_CREATE_FROMFILE;
  FilenameForBPP("LAPTOP\\ammogrid.sti", VObjectDesc.ImageFile);
  CHECKF(AddVObjectAndSetTransparency(CreateVideoObject(&VObjectDesc), &guiAmmoGrid));
