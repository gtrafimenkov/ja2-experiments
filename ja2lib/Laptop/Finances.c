...
BOOLEAN LoadFinances(void) {
  ...
  CHECKF(AddStandardVideoObject(&VObjectDesc, &guiLONGLINE));

  // the records columns
  VObjectDesc.fCreateFlags = VOBJECT_CREATE_FROMFILE;
  FilenameForBPP("LAPTOP\\recordcolumns.sti", VObjectDesc.ImageFile);
  CHECKF(AddStandardVideoObject(&VObjectDesc, &guiLISTCOLUMNS));

  // black divider line - long ( 480 length)
  VObjectDesc.fCreateFlags = VOBJECT_CREATE_FROMFILE;
  FilenameForBPP("LAPTOP\\divisionline.sti", VObjectDesc.ImageFile);
  CHECKF(AddStandardVideoObject(&VObjectDesc, &guiLINE));
...
void DrawSummaryLines(void) {
  ...
  BltVideoObject(vsIndexFB, hHandle, 0, DIVLINE_X, TOP_DIVLINE_Y);
  BltVideoObject(vsIndexFB, hHandle, 0, DIVLINE_X, TOP_DIVLINE_Y + 2);
  BltVideoObject(vsIndexFB, hHandle, 0, DIVLINE_X, BOT_DIVLINE_Y);
  BltVideoObject(vsIndexFB, hHandle, 0, DIVLINE_X, MID_DIVLINE_Y2);
...
void DrawRecordsBackGround(void) {
  ...
  for (int iCounter = 0; iCounter < 35; iCounter++) {
    ...
    BltVideoObject(vsIndexFB, hHandle, 0, TOP_X + 10, TOP_Y + 18 + (iCounter * BLOCK_HEIGHT) + 1);
  }

  // the divisorLines
  GetVideoObject(&hHandle, guiLONGLINE);
  BltVideoObject(vsIndexFB, hHandle, 0, TOP_X + 10, TOP_Y + 17 + (6 * (BLOCK_HEIGHT)));
  GetVideoObject(&hHandle, guiLONGLINE);
  BltVideoObject(vsIndexFB, hHandle, 0, TOP_X + 10, TOP_Y + 19 + (6 * (BLOCK_HEIGHT)));
  GetVideoObject(&hHandle, guiLONGLINE);
  BltVideoObject(vsIndexFB, hHandle, 0, TOP_X + 10, TOP_Y + 19 + ((iCounter) * (BLOCK_HEIGHT)));
