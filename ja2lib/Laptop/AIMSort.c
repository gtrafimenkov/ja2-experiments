...
void RenderAimSort() {
  ...
  BltVideoObjectOld(vsIndexFB, hSortByHandle, 0, AIM_SORT_SORT_BY_X, AIM_SORT_SORT_BY_Y);

  // To MugShots
  GetVideoObject(&hToMugShotHandle, guiToMugShots);
  BltVideoObjectOld(vsIndexFB, hToMugShotHandle, 0, AIM_SORT_TO_MUGSHOTS_X, AIM_SORT_TO_MUGSHOTS_Y);

  // To stats
  GetVideoObject(&hToStatsHandle, guiToStats);
  BltVideoObjectOld(vsIndexFB, hToStatsHandle, 0, AIM_SORT_TO_STATS_X, AIM_SORT_TO_STATS_Y);

  // To Alumni
  GetVideoObject(&hToAlumniHandle, guiToAlumni);
  BltVideoObjectOld(vsIndexFB, hToAlumniHandle, 0, AIM_SORT_TO_ALUMNI_X, AIM_SORT_TO_ALUMNI_Y);
...
void DrawSelectLight(uint8_t ubMode, uint8_t ubImage) {
  ...
  BltVideoObjectOld(vsIndexFB, hSelectLightHandle, ubImage, (AimSortCheckBoxLoc[ubMode]),
