  ...
  if (fCharacterIsMale == TRUE) {
    switch (LaptopSaveInfo.iVoiceId) {
      case (0):
        ...
        CHECKF(AddStandardVideoObject(&VObjectDesc, &uiGraphicHandle));

        // show it
        GetVideoObject(&hHandle, uiGraphicHandle);
        BltVideoObjectOld(vsIndexFB, hHandle, 0, LAPTOP_SCREEN_UL_X + sFaceX,
...
BOOLEAN LoadCharacterPortrait(void) {
  ...
  CHECKF(AddStandardVideoObject(&VObjectDesc, &guiCHARACTERPORTRAIT));
