...
void RenderFlorist() {
  ...
  for (i = 0; i < FLORIST_NUMBER_OF_BULLETS; i++) {
    BltVideoObject(vsIndexFB, hPixHandle, 0, FLORIST_FIRST_BULLET_X, usPosY);
...
void DisplayFloristDefaults() {
  ...
  if (guiCurrentLaptopMode == LAPTOP_MODE_FLORIST) {
    ...
    BltVideoObject(vsIndexFB, hPixHandle, 0, FLORIST_BIG_TITLE_X, FLORIST_BIG_TITLE_Y);
  } else {
    gfHomePageActive = FALSE;
    GetVideoObject(&hPixHandle, guiSmallTitleSymbol);
    BltVideoObject(vsIndexFB, hPixHandle, 0, FLORIST_SMALL_TITLE_X, FLORIST_SMALL_TITLE_Y);
  }
}
