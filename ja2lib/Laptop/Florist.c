...
BOOLEAN EnterFlorist() {
  ...
  CHECKF(AddStandardVideoObject(&VObjectDesc, &guiHandBullet));
...
void RenderFlorist() {
  ...
  for (i = 0; i < FLORIST_NUMBER_OF_BULLETS; i++) {
    BltVideoObjectOld(vsIndexFB, hPixHandle, 0, FLORIST_FIRST_BULLET_X, usPosY);
...
BOOLEAN InitFloristDefaults() {
  ...
  CHECKF(AddStandardVideoObject(&VObjectDesc, &guiFloristBackground));
  ...
  if (guiCurrentLaptopMode == LAPTOP_MODE_FLORIST) {
    ...
    CHECKF(AddStandardVideoObject(&VObjectDesc, &guiLargeTitleSymbol));
  } else {
    // load the leaf back graphic and add it
    VObjectDesc.fCreateFlags = VOBJECT_CREATE_FROMFILE;
    GetMLGFilename(VObjectDesc.ImageFile, MLG_SMALLFLORISTSYMBOL);
    CHECKF(AddStandardVideoObject(&VObjectDesc, &guiSmallTitleSymbol));
...
void DisplayFloristDefaults() {
  ...
  if (guiCurrentLaptopMode == LAPTOP_MODE_FLORIST) {
    ...
    BltVideoObjectOld(vsIndexFB, hPixHandle, 0, FLORIST_BIG_TITLE_X, FLORIST_BIG_TITLE_Y);
  } else {
    gfHomePageActive = FALSE;
    GetVideoObject(&hPixHandle, guiSmallTitleSymbol);
    BltVideoObjectOld(vsIndexFB, hPixHandle, 0, FLORIST_SMALL_TITLE_X, FLORIST_SMALL_TITLE_Y);
  }
}
