...
void RenderFloristOrderForm() {
  ...
  BltVideoObject(vsIndexFB, hPixHandle, 0, FLOWER_ORDER_DELIVERY_LOCATION_X,
                 FLOWER_ORDER_DELIVERY_LOCATION_Y);

  // The flowe Flower Frame
  GetVideoObject(&hPixHandle, guiFlowerFrame);
  BltVideoObject(vsIndexFB, hPixHandle, 0, FLOWER_ORDER_FLOWER_BOX_X, FLOWER_ORDER_FLOWER_BOX_Y);

  // The currenltly selected flwoer
  GetVideoObject(&hPixHandle, guiCurrentlySelectedFlowerImage);
  BltVideoObject(vsIndexFB, hPixHandle, 0, FLOWER_ORDER_FLOWER_BOX_X + 5,
                 FLOWER_ORDER_FLOWER_BOX_Y + 5);

  // The flowe Name Box
  GetVideoObject(&hPixHandle, guiNameBox);
  BltVideoObject(vsIndexFB, hPixHandle, 0, FLOWER_ORDER_NAME_BOX_X, FLOWER_ORDER_NAME_BOX_Y);

  // The flowe Personel sentiments
  GetVideoObject(&hPixHandle, guiPersonalSentiments);
  BltVideoObject(vsIndexFB, hPixHandle, 0, FLOWER_ORDER_SENTIMENT_BOX_X,
...
void DisplayFloristCheckBox() {
  ...
  if (gfFLoristCheckBox0Down)
    BltVideoObject(vsIndexFB, hPixHandle, 1, FLOWER_ORDER_CHECK_BOX_0_X,
                   FLOWER_ORDER_CHECK_BOX_0_Y);
  else
    BltVideoObject(vsIndexFB, hPixHandle, 0, FLOWER_ORDER_CHECK_BOX_0_X,
                   FLOWER_ORDER_CHECK_BOX_0_Y);

  // first check box
  GetVideoObject(&hPixHandle, guiFlowerOrderCheckBoxButtonImage);
  if (gfFLoristCheckBox1Down)
    BltVideoObject(vsIndexFB, hPixHandle, 1, FLOWER_ORDER_CHECK_BOX_1_X,
                   FLOWER_ORDER_CHECK_BOX_1_Y);
  else
    BltVideoObject(vsIndexFB, hPixHandle, 0, FLOWER_ORDER_CHECK_BOX_1_X,
                   FLOWER_ORDER_CHECK_BOX_1_Y);

  // second check box
  GetVideoObject(&hPixHandle, guiFlowerOrderCheckBoxButtonImage);
  if (gfFLoristCheckBox2Down)
    BltVideoObject(vsIndexFB, hPixHandle, 1, FLOWER_ORDER_CHECK_BOX_2_X,
                   FLOWER_ORDER_CHECK_BOX_2_Y);
  else
    BltVideoObject(vsIndexFB, hPixHandle, 0, FLOWER_ORDER_CHECK_BOX_2_X,
                   FLOWER_ORDER_CHECK_BOX_2_Y);

  // third check box
  GetVideoObject(&hPixHandle, guiFlowerOrderCheckBoxButtonImage);
  if (gfFLoristCheckBox3Down)
    BltVideoObject(vsIndexFB, hPixHandle, 1, FLOWER_ORDER_CHECK_BOX_3_X,
                   FLOWER_ORDER_CHECK_BOX_3_Y);
  else
    BltVideoObject(vsIndexFB, hPixHandle, 0, FLOWER_ORDER_CHECK_BOX_3_X,
                   FLOWER_ORDER_CHECK_BOX_3_Y);

  // Foiurth check box
  GetVideoObject(&hPixHandle, guiFlowerOrderCheckBoxButtonImage);
  if (gfFLoristCheckBox4Down)
    BltVideoObject(vsIndexFB, hPixHandle, 1, FLOWER_ORDER_CHECK_BOX_4_X,
                   FLOWER_ORDER_CHECK_BOX_4_Y);
  else
    BltVideoObject(vsIndexFB, hPixHandle, 0, FLOWER_ORDER_CHECK_BOX_4_X,
                   FLOWER_ORDER_CHECK_BOX_4_Y);

  // fifth check box
  GetVideoObject(&hPixHandle, guiFlowerOrderCheckBoxButtonImage);
  if (gfFLoristCheckBox5Down)
    BltVideoObject(vsIndexFB, hPixHandle, 1, FLOWER_ORDER_CHECK_BOX_5_X,
                   FLOWER_ORDER_CHECK_BOX_5_Y);
  else
    BltVideoObject(vsIndexFB, hPixHandle, 0, FLOWER_ORDER_CHECK_BOX_5_X,
                   FLOWER_ORDER_CHECK_BOX_5_Y);
  ...
  switch (ubDropDownMode) {
    ...
    case FLOWER_ORDER_DROP_DOWN_DISPLAY: {
      ...
      for (i = 10; i < FLOWER_ORDER_DROP_DOWN_LOCATION_WIDTH - 10; i += 10) {
        // TOP ROW
        BltVideoObject(vsIndexFB, hImageHandle, 1, i + FLOWER_ORDER_DROP_DOWN_LOCATION_X,
                       usPosY + FLOWER_ORDER_DROP_DOWN_LOCATION_Y);

        // BOTTOM ROW
        BltVideoObject(vsIndexFB, hImageHandle, 6, i + FLOWER_ORDER_DROP_DOWN_LOCATION_X,
      ...
      for (i = 10; i < usHeight - 10; i += 10) {
        BltVideoObject(vsIndexFB, hImageHandle, 3, usPosX + FLOWER_ORDER_DROP_DOWN_LOCATION_X,
      ...
      // top left
      BltVideoObject(vsIndexFB, hImageHandle, 0, 0 + FLOWER_ORDER_DROP_DOWN_LOCATION_X,
                     usPosY + FLOWER_ORDER_DROP_DOWN_LOCATION_Y);
      // top right
      BltVideoObject(vsIndexFB, hImageHandle, 2,
                     ...
                     usPosY + FLOWER_ORDER_DROP_DOWN_LOCATION_Y);
      // bottom left
      BltVideoObject(vsIndexFB, hImageHandle, 5, 0 + FLOWER_ORDER_DROP_DOWN_LOCATION_X,
