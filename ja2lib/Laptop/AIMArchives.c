...
void RenderAimArchives() {
  ...
  for (y = 0; y < ubNumRows; y++) {
    for (x = 0; x < AIM_ALUMNI_NUM_FACE_COLS; x++) {
      // Blt face to screen
      BltVideoObject(vsFB, hFaceHandle, i, usPosX + 4, usPosY + 4, VO_BLT_SRCTRANSPARENCY, NULL);

      // Blt the alumni frame background
      BltVideoObject(vsFB, hFrameHandle, 0, usPosX, usPosY, VO_BLT_SRCTRANSPARENCY, NULL);
...
void DisplayAlumniOldMercPopUp() {
  ...
  BltVideoObject(vsFB, hAlumniPopUpHandle, 0, AIM_POPUP_X, usPosY, VO_BLT_SRCTRANSPARENCY, NULL);
  ...
  for (i = 0; i < ubNumLines; i++) {
    ShadowVideoSurfaceRect(vsFB, AIM_POPUP_X + AIM_POPUP_SHADOW_GAP, usPosY + AIM_POPUP_SHADOW_GAP,
                           AIM_POPUP_X + AIM_POPUP_WIDTH + AIM_POPUP_SHADOW_GAP,
                           usPosY + AIM_POPUP_SECTION_HEIGHT + AIM_POPUP_SHADOW_GAP - 1);
    BltVideoObject(vsFB, hAlumniPopUpHandle, 1, AIM_POPUP_X, usPosY, VO_BLT_SRCTRANSPARENCY, NULL);
    usPosY += AIM_POPUP_SECTION_HEIGHT;
  }
  ...
  ShadowVideoSurfaceRect(vsFB, AIM_POPUP_X + AIM_POPUP_SHADOW_GAP, usPosY + AIM_POPUP_SHADOW_GAP,
                         AIM_POPUP_X + AIM_POPUP_WIDTH + AIM_POPUP_SHADOW_GAP,
                         usPosY + AIM_POPUP_SECTION_HEIGHT + AIM_POPUP_SHADOW_GAP - 1);
  BltVideoObject(vsFB, hAlumniPopUpHandle, 2, AIM_POPUP_X, usPosY, VO_BLT_SRCTRANSPARENCY, NULL);
