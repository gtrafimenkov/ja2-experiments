...
BOOLEAN EnterAimFacialIndex() {
  ...
  CHECKF(AddStandardVideoObject(&VObjectDesc, &guiMugShotBorder));
  ...
  for (y = 0; y < AIM_FI_NUM_MUHSHOTS_Y; y++) {
    for (x = 0; x < AIM_FI_NUM_MUHSHOTS_X; x++) {
      ...
      if (!AddStandardVideoObject(&VObjectDesc, &guiAimFiFace[i])) return (FALSE);
...
BOOLEAN DrawMercsFaceToScreen(uint8_t ubMercID, uint16_t usPosX, uint16_t usPosY, uint8_t ubImage) {
  ...
  BltVideoObjectOld(vsIndexFB, hMugShotBorderHandle, ubImage, usPosX, usPosY);

  // Blt face to screen
  GetVideoObject(&hFaceHandle, guiAimFiFace[ubMercID]);
  BltVideoObjectOld(vsIndexFB, hFaceHandle, 0, usPosX + AIM_FI_FACE_OFFSET,
                    usPosY + AIM_FI_FACE_OFFSET);
  ...
  // else if the merc is currently a POW or, the merc was fired as a pow
    ...
    ShadowVideoSurfaceRect(vsIndexFB, usPosX + AIM_FI_FACE_OFFSET, usPosY + AIM_FI_FACE_OFFSET,
