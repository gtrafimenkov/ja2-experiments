...
void HandleBobbyRMailOrder() {
  ...
  if (gfDrawConfirmOrderGrpahic) {
    ...
    BltVideoObject(vsIndexFB, hPixHandle, 0, BOBBYR_CONFIRM_ORDER_X, BOBBYR_CONFIRM_ORDER_Y);
...
void RenderBobbyRMailOrder() {
  ...
  BltVideoObject(vsIndexFB, hPixHandle, 0, BOBBYR_ORDERGRID_X, BOBBYR_ORDERGRID_Y);

  // Location graphic
  GetVideoObject(&hPixHandle, guiBobbyRLocationGraphic);
  BltVideoObject(vsIndexFB, hPixHandle, 0, BOBBYR_LOCATION_BOX_X, BOBBYR_LOCATION_BOX_Y);

  // DeliverySpeedGraphic
  GetVideoObject(&hPixHandle, guiDeliverySpeedGraphic);
  BltVideoObject(vsIndexFB, hPixHandle, 0, BOBBYR_DELIVERYSPEED_X, BOBBYR_DELIVERYSPEED_Y);

  // Package Weight
  GetVideoObject(&hPixHandle, guiPackageWeightImage);
  BltVideoObject(vsIndexFB, hPixHandle, 0, BOBBYR_PACKAXGE_WEIGHT_X, BOBBYR_PACKAXGE_WEIGHT_Y);
...
void DisplayShippingCosts(BOOLEAN fCalledFromOrderPage, int32_t iSubTotal, uint16_t usGridX,
  ...
  if (fCalledFromOrderPage) {
    ...
    BltVideoObject(vsIndexFB, hPixHandle, 0, BOBBYR_TOTAL_SAVED_AREA_X, BOBBYR_TOTAL_SAVED_AREA_Y);
  }
...
BOOLEAN DrawShippingSpeedLights(uint8_t ubSelected) {
  if (ubSelected == 0)
    ColorFillVideoSurfaceArea(vsIndexFB, gShippingSpeedAreas[0], gShippingSpeedAreas[1],
  ...
  if (ubSelected == 1)
    ColorFillVideoSurfaceArea(vsIndexFB, gShippingSpeedAreas[2], gShippingSpeedAreas[3],
  ...
  if (ubSelected == 2)
    ColorFillVideoSurfaceArea(vsIndexFB, gShippingSpeedAreas[4], gShippingSpeedAreas[5],
...
BOOLEAN CreateDestroyBobbyRDropDown(uint8_t ubDropDownAction) {
  ...
  switch (ubDropDownAction) {
    ...
    case BR_DROP_DOWN_DESTROY: {
      ...
      ColorFillVideoSurfaceArea(vsIndexFB, BOBBYR_SHIPPING_LOC_AREA_L_X,
    ...
    case BR_DROP_DOWN_DISPLAY: {
      ...
      ColorFillVideoSurfaceArea(vsIndexFB, BOBBYR_CITY_START_LOCATION_X,
      ...
      for (i = 10; i < BOBBYR_DROP_DOWN_WIDTH - 10; i += 10) {
        // TOP ROW
        BltVideoObject(vsIndexFB, hImageHandle, 1, i + BOBBYR_CITY_START_LOCATION_X,
                       usPosY + BOBBYR_CITY_START_LOCATION_Y);
        // BOTTOM ROW
        BltVideoObject(vsIndexFB, hImageHandle, 6, i + BOBBYR_CITY_START_LOCATION_X,
      ...
      for (i = 10; i < usHeight - 10; i += 10) {
        BltVideoObject(vsIndexFB, hImageHandle, 3, usPosX + BOBBYR_CITY_START_LOCATION_X,
                       i + BOBBYR_CITY_START_LOCATION_Y);
        BltVideoObject(vsIndexFB, hImageHandle, 4,
      ...
      // top left
      BltVideoObject(vsIndexFB, hImageHandle, 0, 0 + BOBBYR_CITY_START_LOCATION_X,
                     usPosY + BOBBYR_CITY_START_LOCATION_Y);
      // top right
      BltVideoObject(vsIndexFB, hImageHandle, 2,
                     BOBBYR_DROP_DOWN_WIDTH - 10 + BOBBYR_CITY_START_LOCATION_X,
                     usPosY + BOBBYR_CITY_START_LOCATION_Y);
      // bottom left
      BltVideoObject(vsIndexFB, hImageHandle, 5, 0 + BOBBYR_CITY_START_LOCATION_X,
                     usHeight - 10 + BOBBYR_CITY_START_LOCATION_Y);
      // bottom right
      BltVideoObject(vsIndexFB, hImageHandle, 7,
                     BOBBYR_DROP_DOWN_WIDTH - 10 + BOBBYR_CITY_START_LOCATION_X,
                     usHeight - 10 + BOBBYR_CITY_START_LOCATION_Y);
      ...
      ColorFillVideoSurfaceArea(vsIndexFB, BOBBYR_SCROLL_AREA_X, BOBBYR_SCROLL_AREA_Y,
      ...
      for (i = 10; i < BOBBYR_SCROLL_AREA_HEIGHT - 10; i += 10) {
        BltVideoObject(vsIndexFB, hImageHandle, 3,
      ...
      for (i = 0; i < BOBBYR_SCROLL_AREA_WIDTH; i += 10) {
        // TOP ROW
        BltVideoObject(vsIndexFB, hImageHandle, 1, i + BOBBYR_SCROLL_AREA_X - 10,
                       BOBBYR_SCROLL_AREA_Y);
        // BOTTOM ROW
        BltVideoObject(vsIndexFB, hImageHandle, 6, i + BOBBYR_SCROLL_AREA_X - 10,
      ...
      // top arrow
      BltVideoObject(vsIndexFB, hArrowHandle, 1, BOBBYR_SCROLL_UP_ARROW_X,
                     BOBBYR_SCROLL_UP_ARROW_Y);

      // top arrow
      BltVideoObject(vsIndexFB, hArrowHandle, 0, BOBBYR_SCROLL_DOWN_ARROW_X,
...
void DrawSelectedCity(uint8_t ubCityNumber) {
  ...
  ColorFillVideoSurfaceArea(vsIndexFB, BOBBYR_CITY_START_LOCATION_X + 4, usPosY + 4,
...
void DisplayShippingLocationCity() {
  ...
  // display the name on the title bar
  ColorFillVideoSurfaceArea(vsIndexFB, BOBBYR_SHIPPING_LOC_AREA_L_X, BOBBYR_SHIPPING_LOC_AREA_T_Y,
...
void DrawGoldRectangle(int8_t bCityNum) {
  ...
  ColorFillVideoSurfaceArea(vsIndexFB, BOBBYR_SCROLL_AREA_X, usPosY, BOBBYR_SCROLL_AREA_X + usWidth,
...
BOOLEAN CreateBobbyRayOrderTitle() {
  ...
  CHECKF(AddStandardVideoObject(&VObjectDesc, &guiBobbyRayTitle));
...
void DrawBobbyROrderTitle() {
  ...
  BltVideoObject(vsIndexFB, hPixHandle, 0, BOBBYR_BOBBY_RAY_TITLE_X, BOBBYR_BOBBY_RAY_TITLE_Y);
}
