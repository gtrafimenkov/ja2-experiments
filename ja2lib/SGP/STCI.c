...
static BOOLEAN STCILoadIndexed(struct Image *hImage, bool loadAppData, FileID hFile,
                               struct STCIHeader *pHeader);
static BOOLEAN STCISetPalette(void *pSTCIPalette, struct Image *hImage);
...
static BOOLEAN STCILoadRGB(struct Image *hImage, bool loadAppData, FileID hFile,
  ...
  uint16_t fContents = IMAGE_ALLIMAGEDATA;
  if (loadAppData) {
  ...
  // XXX: read head.StoredSize bytes to hImage->pImageData
  {
    ...
    if (pHeader->end.Depth == 16) {
      ...
      if (gusRedMask != (uint16_t)pHeader->middle.rgb.uiRedMask ||
          gusGreenMask != (uint16_t)pHeader->middle.rgb.uiGreenMask ||
          gusBlueMask != (uint16_t)pHeader->middle.rgb.uiBlueMask) {
...
static BOOLEAN STCILoadIndexed(struct Image *hImage, bool loadAppData, FileID hFile,
  ...
  if (fContents & IMAGE_APPDATA && pHeader->end.AppDataSize > 0) {
    ...
    hImage->pAppData = (uint8_t *)MemAlloc(pHeader->end.AppDataSize);
